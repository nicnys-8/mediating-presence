...
function corr_coeff(data, terms) {
  var r = 0;
  var n = data.length;
  var sx = 0;
...
function compute_coefficients(data,  p) {
  p += 1;
  var n = data.length;
  var r, c;
  var rs = 2 * p - 1;
...
// p = polynomial degree
function process_data(data,p) {
  var terms = compute_coefficients(data,p);
  var cc = corr_coeff(data,terms);
  var se = std_error(data,terms);
  return [terms,cc,se];
}
